...
const IndustryPresence = () => {
  ...
  const data: Record<TabKey, { title: string; subtitle: string; img: string }[]> = {
    ...
    "Booth highlights": [
      { title: "Booth Highlight 1", subtitle: "Location here", img: "/booth1.png" },
      { title: "Booth Highlight 2", subtitle: "Location here", img: "/booth2.png" },
    ],
    "Media coverage": [
      { title: "Media Coverage 1", subtitle: "Location here", img: "/media1.png" },
      { title: "Media Coverage 2", subtitle: "Location here", img: "/media2.png" },
    ],
  };
  ...
  // Hover auto-slide — starts sliding forward, wraps around
  const onCarouselMouseEnter = () => {
    if (isMobile || maxIndex === 0) return;
    setIsHovering(true);
    hoverIntervalRef.current = setInterval(() => {
      setCurrent((c) => {
        if (c >= maxIndex) return 0; // wrap to start
        return c + 1;
      });
    }, 1800);
  };
  ...
  return (
    <Grid size={12} sx={{ pt: 8 }}>
      ...
      >
        {(Object.keys(data) as TabKey[]).map((tab) => (
          <Button
            ...
            sx={{
              border: active === tab ? "1px solid #F6891F" : "1px solid rgba(0,0,0,0.2)",
      ...
      >
        {/* ── LEFT ARROW — shown on both mobile & desktop ── */}
        ...
        {isMobile ? (
          <Box sx={{ overflow: "hidden", px: `${MOBILE_PEEK_VW}vw` }}>
            ...
            >
              {cards.map((card, index) => (
                <CardContent key={index} card={card} index={index} isMob={true} />
              ))}
            </Box>
          </Box>
        ) : (
          ...
          <Box sx={{ overflow: "hidden" }}>
            ...
            >
              {cards.map((card, index) => (
                <CardContent key={index} card={card} index={index} isMob={false} />
              ))}
            </Box>
          </Box>
        )}

        {/* ── RIGHT ARROW — shown on both mobile & desktop ── */}
        {cards.length > CARDS_PER_VIEW && (
          <IconButton
            onClick={handleNext}
            disabled={!canNext}
